...
import { costDataGeneratorAction } from "../../_actions";
...
class CostDataGenerator extends Component {
  constructor(props) {
    super(props);
    this.state = {
      startDate: "",
  ...
  handleCostData = async (event) => {
    const { startDate, endDate } = this.state;
    event.preventDefault();
    this.setState({
      isSubmitted: true
    });
    const newErrorData = this.validateCostData(true);
    if (newErrorData.isValid) {
      var data = {
        "startDate": startDate,
        "endDate": endDate
      };
      this.props.dispatch(costDataGeneratorAction.getCostDataGenerator(data));
    }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.cost_data_status !== prevProps.cost_data_status && this.props.cost_data_status == status.SUCCESS) {
      if (this.props.cost_data_list && this.props.cost_data_list.length > 0) {
        this.setState({
          costList: this.props.cost_data_list
        })
      }
    }
  }
    ...
    return (
      <>
        <div className='form-container'>
          {
            OnBoardingFormData.form.cost_data.map(costDetails => {
              return (
                <>
                              ...
                              <div className="row">
                                <div className="col-xl-9 col-lg-9 col-md-8 col-sm-12 col-12">
                                  <div className="user-content">
                                    <div className="row">
                                      {
                                        costDetails.fields.map(fieldsData => {
                                          return (
                                ...
                                <Container className="mt-4" >
                                  <Tooltip title="Save">
                                    <Button type="submit" className="ml-0" variant="contained" color="primary" onClick={this.handleCostData}>
          ...
          {/* <div className="main-content">
            <div className="vendor-content">
              ...
              <div style={{ height: 400, width: '100%', marginTop: "12px" }}>
                <DataGrid
                  rows={this.props.cost_data_list != null && this.props.cost_data_list}
                  columns={orgCloumn}
                  initialState={{
                    pagination: {
...
function mapStateToProps(state) {
  const { cost_data_status, cost_data_list } = state.hr;
  return {
    cost_data_status,
    cost_data_list
  };
...
const connectedCostDataGenerator = connect(mapStateToProps)(CostDataGenerator);
export default connectedCostDataGenerator;
