...
import { departmentAction, landingzoneAction, organizationAction } from "../../_actions";
...
class Landingzone extends Component {
    ...
    handleColumnReorder = (newColumns) => {
        this.setState({ lzCloumn: newColumns });
    };
    handleStateChange = (e) => {
    ...
    handleLandingzone = async (event) => {
        const { organization, department, landingzone, cloud, displayName, roleArn, externalId } = this.state;
        event.preventDefault();
        this.setState({
            isSubmitted: true
        });
        const newErrorData = this.validateLz(true);
        if (newErrorData.isValid) {
            ...
            this.props.dispatch(landingzoneAction.addLandingzone(newLandingzone));
        }
    }
    ...
    exportToExcel = () => {
        if (this.props.landingzone_list && this.props.landingzone_list.length > 0) {
            ...
            const fileName = 'landingzone';
            const ws = XLSX.utils.json_to_sheet(this.props.landingzone_list);
    ...
    render() {
        const { orgList, lzCloumn } = this.state;
        return (
            <>
                <div className='form-container'>
                    {
                        OnBoardingFormData.form.landingZone.map(landingZoneDeatils => {
                            return (
                                <>
                                                    ...
                                                    <div className="general-contect">
                                                        <div className="d-flex heading">
                                                            <h5 className="d-inline-block">
                                                                {landingZoneDeatils.landingZone_title}
                                                            </h5>
                                                        </div>
                                                            ...
                                                            <div className="row">
                                                                <div className="col-xl-9 col-lg-9 col-md-8 col-sm-12 col-12">
                                                                    <div className="user-content">
                                                                        <div className="row">
                                                                            {
                                                                                landingZoneDeatils.fields.map(fieldsData => {
                                                                                    return (
                                                                                            ...
                                                                                            <div className="form-group form-group-common">
                                                                                                {
                                                                                                    fieldsData.html_element === "textbox" ?
                                                                                                        ...
                                                                                                        :
                                                                                                        fieldsData.html_element === "textbox" && fieldsData.name === "cloud" ?
                                                                                                            <>
                                                                                                                <label className="d-block">{fieldsData.lable}</label>
                                                                                                                <input type={fieldsData.html_element} value={"aws"} disabled name={fieldsData.name} required={fieldsData.required} datatype={fieldsData.data_type} onChange={this.handleStateChange}
                                                                                                                    placeholder="" className="form-control" />
                                                                                                            </>
                                                                ...
                                                                <Container className="mt-4" >
                                                                    <Tooltip title="Save">
                                                                        <Button type="submit" className="ml-0" variant="contained" color="primary" onClick={this.handleLandingzone}>
                    ...
                    <div className="main-content">
                        <div className="vendor-content">
                            <div className="d-flex">
                                <h4>Landing Zone Table</h4>
                                <Button variant="contained" className="btnData ml-4" style={{ backgroundColor: "#16619F", color: "white", borderRadius: "30px" }} onClick={this.refreshlandingZone}>
                            ...
                            <div className="mt-3" style={{ height: 400, width: '100%' }}>
                                <DataGrid
                                    rows={this.props.landingzone_list != null && this.props.landingzone_list}
                                    columns={lzCloumn}
                                    components={{
                                        Toolbar: GridToolbar,
                                      }}
...
function mapStateToProps(state) {
    const { organization_status, organization_list, department_status, department_list, landingzone_status, landingzone_list } = state.hr;
    return {
        organization_status,
        organization_list,
        department_status,
...
const connectedLandingzone = connect(mapStateToProps)(Landingzone);
export default connectedLandingzone;
